...
//依存するモデルクラスファイルを読み込む
include_once("{$projectDirPath}/apps/work/list/model.php");
...
class WorkListController extends ControllerCore
{
    private $model;
    ...
    public function action()
    {
        ...
        //モデルを生成
        $this->model = new WrokListModel($this->dataStore);

        //smartyに変数をアサイン
        $this->viewSmarty->assign(array(
            'workList'          => $this->model->getWorkList(),
            'footerHtml'        => $commonFooterHtml,
            'headerHtml'        => $commonHeaderHtml,
        ));

        //ビューHTMLの文字列を返す
        return $this->viewSmarty->fetch('../apps/work/list/list.html');
    }
}
